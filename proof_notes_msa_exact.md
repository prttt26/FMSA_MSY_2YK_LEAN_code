...
| MSAEMIX.6 ⭐⭐ (**NEW 2026-08-25**) | the **matrix Blum–Høye root is `C^∞` at `K = 0`** — the general-`N` analogue of `exists_contDiffAt_bhRoot`, i.e. the hypothesis of paper Theorem I.1 for mixtures | ✅ **ALL 5 ITEMS DONE (2026-08-25, pushed, all std-3):** item 1 `exists_contDiffAt_root_of_prodDomain_ift` (`81f8c32`); item 2 `matBhResidual`+`_eq_zero_iff` (`643136d`); item 3 `contDiff_matBhResidual` (`8252071`); item 4 `matBhResidual_base_eq`+`exists_contDiffAt_matBhRoot` (`5a36c38`); item-5 **engine** `matBhJacobianCLM_isInvertible`+`…_of_blockJacobian` (`2194180`); item-5 **transcription** `matBhResidualShape_hasFDerivAt` (`0cf4c01`) + concrete layer `resG/resH/resD` (`matBhResidual_zero_eq`, `contDiff_res*`, `res{H,D}_fderiv_inr`) assembled into ⭐ **`exists_contDiffAt_matBhRoot_physical`** (`007fb7b`): the general-`N` mixture MSA amplitudes are a `C^∞` family near `K=0` from a `MixBHRoot` seed + the two HS-OZ blocks `resG (0,Gt₀)`,`resH (0,Gt₀)` nonsingular. Nothing left as a hypothesis except the two physical dets — the exact matrix analogue of `N = 1`'s `F₀ ≠ 0` (conditional: `Q0_mat_phys_isUnit_det_of_diag_dom`; the unconditional `Q0_mat_phys_isUnit_det` is retired-as-false). Same "two of three parts" grade as `N = 1` (transcription faithfulness of the BH system stays open) |
...
the assembly is now `exists_contDiffAt_matBhRoot_physical` (`007fb7b`), general `N`, std-3, with the
fderiv transcription proved internally and only the two physical HS-block dets left as hypotheses
(the matrix `F₀ ≠ 0`). Theorem I.1 stops being conditional for mixtures modulo the transcription part.
