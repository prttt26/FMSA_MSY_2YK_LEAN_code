...
| MSAEMIX.4 ⭐ (**the matrix ring**) | discharge the matrix `hcore` — `matEMIXfactorization_of_core`'s closure-recovery ring | ☐ **OPEN — the matrix analog of MSAEXACT.6; axiom-landing path fixed 2026-08-22.** MSAEXACT.6 is now an axiom (`msaexact6_hcore`); MSAEMIX.4's sound axiom-landing mirrors the scalar and needs the SAME two ingredients the scalar had ready: **(1) assemble `matMSACoreCorr`** — the matrix closure core, per-entry `coreCorrection` with `c₁..c₅` from the matrix amplitudes (`Qppphys/Q0phys` HS base + `mixDA/mixDqp` increments + `gam`, generalised to `σ_ij`), the same formula as scalar `msaCoreCorr`; **(2) a matrix `hcore` certificate** (unequal-σ analog of `msaexact6_cert.py`) to VERIFY it before axiomatizing. ⚠ **do NOT axiomatize without the cert** — an unequal-σ slip ⇒ unsound; defining `cMSAhat` from the increment ⇒ tautological. Q-side DONE (`Q0_mat_c_sub`+`mixM/N/DA/Dqp`, validated 1e-15); c-side = the remaining work. Two sub-layers as before: per-entry engine (= MSAEXACT.6 unequal-σ) + the ∑_l Wiener–Hopf/Khinchin collapse (`matBaxterFourierWH_of_wk_sym` analog; HS: hWK+`ring_nf;linarith`). MSAEMIX.1 is meanwhile **soundly reduced** to this explicit `hcore` (no unsound axiom) |
