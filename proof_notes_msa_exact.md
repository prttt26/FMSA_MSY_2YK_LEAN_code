...
| MSAEMIX.4 ⭐ (**the matrix ring**) | discharge the matrix `hcore` — `matEMIXfactorization_of_core`'s closure-recovery ring | ☐ **OPEN — the matrix analog of MSAEXACT.6.** Two layers: (i) a **per-entry** polynomial identity = the **same engine** as MSAEXACT.6 (the multiplier), generalised to unequal-σ pairs (λ_ij shifts); (ii) the **∑_l Wiener–Hopf/Khinchin collapse** (`(Q₀·Q₁ᵀ)_ij=∑_l …` → entry-wise DCF) — a Fourier-analysis/Fubini task = the `hWK`-with-tails analog of `HSMixture/MatrixBaxterFourierWH.matBaxterFourierWH_of_wk_sym` (HS: hWK + `ring_nf;linarith`, NOT a second multiplier). ⚠ equal-σ likely √-collapses to the scalar (MRS.8/MIXCHS.7-N pattern); the new content is unequal-σ ∑_l. **Do MSAEXACT.6 first** (shared engine); MSAEMIX.4 = engine-reuse + the collapse layer |
