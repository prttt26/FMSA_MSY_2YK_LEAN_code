...
> (`msaMixture_reduces_to_scalar_at_fin_one`); **.4/.5** the core + breakpoint structure; **.7** the first-order-equivalence analog of MSAEXACT.5 (`firstOrder_amplitude_eq_hardSphere_dressed_matrix`, axiom-clean). (**.6**, the matrix BH-root smoothness, is a separate OPEN task — see the REOPENED note.) **No open math
...
>
> ⚠ **REOPENED 2026-08-25 with one new task, MSAEMIX.6.** "Fully closed" was true of the
> *factorization* chain and is still true of it. MSAEMIX.6 is a different statement — the
> **differentiability of the solution family at zero coupling, at general `N`** — which the
> group never carried because it lived on the FOEQ/MSAFAM side at `N = 1`. It is what the
> paper's Theorem I.1 is conditional on for mixtures.
...
### ✅ MSAEMIX.6 — the matrix BH root is `C^∞` at `K = 0` (general `N`) — ALL 5 ITEMS DONE
...
Baxter factor at `s = z`). ⚠ **Correction (2026-08-25): `Q0_mat_phys` invertibility is NOT
unconditional.** The only theorem is `Q0_mat_phys_isUnit_det_of_diag_dom` (conditional on strict row
diagonal dominance); the previous "`det Q̂₀ ≠ 0` for every physical mixture at every `N`" was a
**stale/false claim** (the file's own docstring records the unconditional statement was retired as
false — adversarial `Qp` forces `det = 0`). So the matrix `F₀ ≠ 0` analogue is a *conditional* input
(`IsUnit N₁.det`, `IsUnit N₂.det`), exactly as `N = 1`'s `F₀ ≠ 0` is discharged by the concrete
`Q0_ne_zero_at_yukawa` — the mixture just has no unconditional discharge yet.

**Work items** — items 1–4 + the item-5 engine DONE; only the item-5 fderiv transcription remains:
...
transcription proved internally; and the two physical HS-block dets are **discharged** (`70c35e6`)
onto `Q0_mat_phys` diagonal dominance (`exists_contDiffAt_matBhRoot_of_diag_dom`), the mixture `η∈(0,1)`.
Theorem I.1 stops being conditional for mixtures modulo the transcription-faithfulness part only.
