...
| MSAEMIX.2 | `msaMixture_reduces_to_scalar_at_fin_one` — the `N = 1` specialisation **is** MSAEXACT.1 | ◑ *positivity* half done (`mixCompressibility_fin_one`, `mixStability_fin_one`); **factorization half's `N=1` bridge DONE 2026-08-20** (`matEMIX_factorization_fin_one_iff`/`matEMIXfactorization_of_core_fin_one`, `MSAEMixFactorization.lean`): at `N=1` the matrix factorization for a conjugate-pair factor `A∓iB` **is** the scalar Baxter modulus `A²+B²=1−ρĉ` — the `factorization_of_core` shape (`A=1−ρReQ̂`, `B=ρImQ̂`). Full reduction still waits on the concrete matrix `hcore` (MSAEMIX.1) |
