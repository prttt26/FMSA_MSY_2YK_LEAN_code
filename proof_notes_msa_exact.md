...
| MSAEMIX.6 ⭐⭐ (**NEW 2026-08-25**) | the **matrix Blum–Høye root is `C^∞` at `K = 0`** — the general-`N` analogue of `exists_contDiffAt_bhRoot`, i.e. the hypothesis of paper Theorem I.1 for mixtures | ◑ **items 1–2 DONE (2026-08-25, pushed):** item 1 `exists_contDiffAt_root_of_prodDomain_ift` generalised to any complete `E` (`81f8c32`); item 2 `matBhResidual` + `matBhResidual_eq_zero_iff` (zero set = `MixBHRoot`, `MSAMixtureBHRootSmooth.lean`, std-3, `643136d`). **Items 3–5 + capstone OPEN** — need `Matrix` Frobenius norm activated first, then from-scratch matrix ContDiff (item 3), the Fréchet block-triangular Jacobian (item 5), HS seed `Gt₀` (item 4). Scoped below: 4 mechanical items + 1 with content (the matrix Jacobian). ⭐ The base-point non-degeneracy is **already a theorem** (`FMSA.MatrixQ0.Q0_mat_phys_isUnit_det`, any `N`, std-3), so this is short of assembly, not of an ingredient |
...
**⭐ The hard input is already in hand.** At `N = 1` the Jacobian step is
`bhJacobianCLM_isInvertible`, whose docstring records the shape that matters: the Jacobian is
**block-triangular**, and nonsingular ⟺ `F₀ ≠ 0` with `F₀` the hard-sphere Baxter factor. The
matrix analogue of `F₀ ≠ 0` is `FMSA.MatrixQ0.Q0_mat_phys_isUnit_det` — `det Q̂₀ ≠ 0` for every
physical mixture at every `N`, `#print axioms` = std-3 (recorded in `FMSAProject.lean`). So the
non-degeneracy is a theorem; what is missing is the assembly around it.

**Work items** — four mechanical, one with content:
...
| 3 | `ContDiff` of `g` — entries are polynomial in the amplitudes plus `Real.exp` of constants. ⚠ matrix-valued smoothness is fiddlier than the scalar `msaQre_contDiff`/`msaQim_contDiff`; expect `fun_prop` plus per-entry plumbing | **medium** |
| 4 | Base equation: at `K = 0` the HS amplitudes `(0, Gt₀)` solve the pair — the matrix `bh_base_eq` | **medium** |
| 5 | ⭐ **Jacobian invertibility at the base point.** Show the matrix Jacobian is block-triangular with diagonal blocks given by multiplication by `Q̂₀`, then discharge with `Q0_mat_phys_isUnit_det`. At `N = 1` the corresponding `D 0 = 0` argument is MSAEXACT.5's `firstOrder_amplitude_eq_hardSphere_dressed` — ⚠ that one is stated for **scalar** `D F : ℝ → ℝ`, so it is a template, not a lemma to reuse | **the real work** |
...
**Paper effect.** §I C's sentence *"What the mixture still lacks is the assembly …"* retires,
and Theorem I.1 stops being conditional for mixtures modulo the transcription part.
