...
| MSAEMIX.6 ⭐⭐ (**NEW 2026-08-25**) | the **matrix Blum–Høye root is `C^∞` at `K = 0`** — the general-`N` analogue of `exists_contDiffAt_bhRoot`, i.e. the hypothesis of paper Theorem I.1 for mixtures | ◑ **items 1–4 + item-5 engine + capstone DONE (2026-08-25, pushed, all std-3):** item 1 `exists_contDiffAt_root_of_prodDomain_ift` → any complete `E` (`81f8c32`); item 2 `matBhResidual` + `_eq_zero_iff` (`643136d`); item 3 `contDiff_matBhResidual` (`8252071`); item 4 `matBhResidual_base_eq` + capstone `exists_contDiffAt_matBhRoot` (`5a36c38`); item-5 **engine** `matMulRightCLM`/`matBhJacobianCLM`/`matBhJacobianCLM_isInvertible` + `exists_contDiffAt_matBhRoot_of_blockJacobian` (`2194180`). **Remaining = item-5 fderiv-shape transcription alone** — `fderiv (matBhResidual) ∘L inr = matBhJacobianCLM N₁ N₂ Cpl`, the matrix analogue of `bhResidualShape_hasFDerivAt` + `MSABlumHoyeSystem` fderivs (a dedicated-file effort, transcription grade). ⚠ **Correction:** the diagonal-block invertibility (`IsUnit N₁.det`/`N₂.det`) is **conditional** (`Q0_mat_phys_isUnit_det_of_diag_dom`), NOT the retired-as-false unconditional `Q0_mat_phys_isUnit_det`; exposed honestly as a hypothesis, exactly like `N = 1`'s `F₀ ≠ 0` |
...
### ☐ MSAEMIX.6 — the matrix BH root is `C^∞` at `K = 0` (general `N`)
...
| 5 | ◑ **engine DONE** (`2194180`, std-3), transcription OPEN. **Engine:** `matMulRightCLM` (`M ↦ M·N`), `matBhJacobianCLM N₁ N₂ Cpl` (block lower-triangular), `matBhJacobianCLM_isInvertible` (units `N₁, N₂` ⇒ invertible; `Cpl` drops out) = the general-`N` analogue of `bhJacobianCLM_isInvertible`; `exists_contDiffAt_matBhRoot_of_blockJacobian` discharges the capstone's `hjac` from (i) the **shape** `fderiv g ∘L inr = matBhJacobianCLM N₁ N₂ Cpl` and (ii) `IsUnit N₁.det`, `IsUnit N₂.det`. **Remaining = the fderiv transcription (i)** — the matrix analogue of `bhResidualShape_hasFDerivAt` + the concrete amplitude fderivs (`MSABlumHoyeSystem.lean`), a dedicated-file effort. Structure verified informally: R1's `Dt·dG` term vanishes at `Dt=0`; R2's Gt-directional derivative is exactly `2π·dGt·N₂` because every dressing deviation is `∝ Dt` so its Gt-partial vanishes at base | **the real work (engine done, transcription open)** |

**⚠ What it will and will not buy.** Exactly as at `N = 1`, this proves the statement for the
*transcribed* Blum–Høye system. Faithfulness of the transcription is the third part that
`proof_notes` and the paper's not-formalized section both still list as open. So on landing,
the honest claim becomes "two of three parts, at general `N`" — the same grade `N = 1` has now,
not a stronger one.

**Paper effect.** §I C's sentence *"What the mixture still lacks is the assembly …"* — the *assembly*
(items 1–4 + the invertibility engine) is now in hand as `exists_contDiffAt_matBhRoot` /
`…_of_blockJacobian`; what remains is the fderiv-shape transcription, at the acknowledged
transcription grade.
