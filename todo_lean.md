...
|------|-------|-----------|-------|
| `MSAEXACT.1–6` *(msa_exact)* | Exact MSA closed form, single component | MSAX.1 gate 0 ✓ | ◑ **.1–.5 DONE** (`.1`=`exactMSA_factorization` via the `exactMSA_hcore` axiom; `.2`/`.4` done; `.3` core, `βK≈3` measured); the group's **sole open piece is `.6`** — the `hcore` ring, `ring`-INFEASIBLE (measured 2026-08-22; Fourier layer discharged out-of-build via `ExactMSACertificate` to 1 pure-algebra axiom `exactMSA_kspace_residual`). ⛔ compact `exactMSA_iff_core` route dead. → [msa_exact](proof_notes_msa_exact.md) |
| `MSAEMIX.0–7` *(msa_exact)* | Same at general `N` (matrix) | UNGATED 2026-08-19 | ✅ **GROUP CLOSED** — `.0`/`.3` axiom-clean; `.1`=`matMSAmixture_{equalDiam,unequalDiam_of_hcore}` (modulo the mixture `hcore` axioms, analog of MSAEXACT.6); `.2`=`msaMixture_reduces_to_scalar_at_fin_one` (both halves, 2026-08-25); `.4`/`.5` core+breakpoints; `.6` matrix BH-root `C^∞` **both diameters, unconditional (2026-09-01)**; `.7` first-order-equiv analog. → [msa_exact](proof_notes_msa_exact.md) |
