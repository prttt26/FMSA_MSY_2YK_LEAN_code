...
| MSAFAM.5 ⭐⭐ | `msaSolutionFamily` — from the root `ψ(τ)` to `H C : ℝ → ℂ` at fixed real `k`, with `ContDiff` and `∀ᶠ τ, (1+H τ)(1−C τ) = 1` | **MSAEXACT.1 (full)**; MSAFAM.1–4 | ◑ **transport half DONE 2026-08-19** (`MSASolutionFamily.msaSolutionFamily_transport`, axiom-clean): given the Baxter transform `τ↦(Q̂(k,τ),Q̂(−k,τ))` `ContDiff` at 0 with `Q̂(k,0)Q̂(−k,0)≠0`, the pair `C̃=1−Q̂Q̂`, `H̃=(1−C̃)⁻¹−1` is `ContDiff` + obeys OZ `∀ᶠ` — the three FOEQ.7/8 hyps. **Remaining = the identity half only**: the concrete `ContDiff` family `τ↦Q̂(±k,ψ(τ))` (k-space Baxter transform of `msaBaxterFn` along the MSAFAM.3 root `ψ`) + `Q̂(k,0)≠0` = **MSAEXACT.1's open closure-recovery core** |
| MSAFAM.6 ⭐⭐ | Theorem I.1 with **no hypothesis**: `oz_msa_taylor_eq_hierarchy` applied to the constructed family, `N = 1` | MSAFAM.2/3/4/5 | ◑ **abstract form DONE 2026-08-19** (`MSASolutionFamily.msaFamily_taylor_eq_hierarchy`, axiom-clean): the transport family's Taylor coeffs satisfy the hierarchy at **every** `m≥1` — FOEQ.7 composed with `msaSolutionFamily_transport`; ⚠ MSAFAM.4's `∀ᶠ` weakening is what makes this typecheck (the family folds ⇒ only local OZ). Everything downstream of the concrete `τ↦Q̂(±k,ψ(τ))` input (= MSAEXACT.1's k-space transform, `Q̂(k,0)≠0`) is now assembled; the capstone lands when that single input is supplied |
