...
| PYE.4 | Abstract equivalence: `DP-map[g_HS·(−βu)] = OZ+PY first order` | ⭐ **CAPSTONE ASSEMBLED 2026-09-01 → complete conditional theorem** (`dp_converges_to_py_first_order_of_whProjectionL1`, `Closures/DPClosureMap.lean`, std-3, gated §PYE in `ClosuresProject.lean`): the whole equivalence is now ONE theorem — chains `exists_tailFit_whConvergent` (the `L¹` approximants + their WH convergence) → `dp_tendsto_py_first_order` (the (★) assembly's continuity), concluding the finite-tail DP DCFs converge entrywise to the exact first-order PY DCF. **Sole hypothesis beyond the physical set-up is `WHProjectionL1`** (the matrix Wiener-`1/f` `L¹` bound; `map_sub`+`bound` fields, non-vacuity certified). Prior state: ◑ substantially reduced (2026-08-03) — `TailFitWHConvergent` *derived* from `L¹` convergence + `WHProjectionL1`. The remaining `WHProjectionL1` is an MA.13/MA.15-class analysis boundary — **deliberately a `Prop`/hypothesis, not a duplicated axiom**; same grade as the 14 registered axioms (Mathlib has no Wiener algebra for the causal-convolution `Q̂₀⁻¹` kernel). Assembly adds **no** axiom |
...
| MSAFAM.5 ⭐⭐ | `msaSolutionFamily` — from the root `ψ(τ)` to `H C : ℝ → ℂ` at fixed real `k`, with `ContDiff` and `∀ᶠ τ, (1+H τ)(1−C τ) = 1` | **MSAEXACT.1 (full)**; MSAFAM.1–4 | ◑ **transport half DONE 2026-08-19** (`MSASolutionFamily.msaSolutionFamily_transport`, axiom-clean): given the Baxter transform `τ↦(Q̂(k,τ),Q̂(−k,τ))` `ContDiff` at 0 with `Q̂(k,0)Q̂(−k,0)≠0`, the pair `C̃=1−Q̂Q̂`, `H̃=(1−C̃)⁻¹−1` is `ContDiff` + obeys OZ `∀ᶠ` — the three FOEQ.7/8 hyps. **⭐ concrete family now BUILT 2026-08-20** (`Closures/MSAFamilyConcrete.lean`, axiom-clean): `exists_contDiffAt_bhRoot` gives the `C^∞` root `ψ` for the concrete `bhF`/`bhP` (prod-domain IFT on FOEQ.5's map, whole pair); `Qp/Qm=1−ρQ̂(±ik,ψ)` `ContDiff` via `msaQre/msaQim_contDiff` ∘ ψ; `Q̂(k,0)≠0` from `one_sub_rho_radial_fourier_c_HS_pos` (no-spinodal). **Remaining = only the *identity* certification** `C̃=ρĉ_MSA` = **MSAEXACT.1's `factorization_of_core` + its open `hcore` ring** |
