...
`detF_mixHS_summable` feeds to `mixHSterm` is exactly the residue of the off-diagonal inverse entry
...
`mixHS_summable_of_growth` reduction via the `detF_family_magnitude_bound` gate.  The third clause
