...
/-- **φ4 formula:** `∫0^sigma r^4 exp(-sr) dr`
`= (24 - (24 + 24ssigma + 12s^2sigma^2 + 4s^3sigma^3 + s^4sigma^4) exp(-ssigma)) / s^5`
...
theorem phi4_formula {s : ℝ} (hs : s ≠ 0) (sigma : ℝ) :
    ...
    (24 - (24 + 24 * s * sigma + 12 * s ^ 2 * sigma ^ 2 +
       4 * s ^ 3 * sigma ^ 3 + s ^ 4 * sigma ^ 4) *
...
/-- **Task OZ.3 definition:** Laplace-weighted inner-core integral using the PY polynomial form:
    ...
    C_HS_laplace(eta, sigma, s)
      = ∫0^sigma r · (-alpha0 - alpha1·(r/sigma) - alpha3·(r/sigma)^3) · e^{-sr} dr
...
theorem C_HS_laplace_formula {eta sigma s : ℝ} (hsigma : 0 < sigma) (hs : s ≠ 0) :
    ...
    -(py_a1 eta / sigma) *
      (2 - (2 + 2 * s * sigma + s ^ 2 * sigma ^ 2) * Real.exp (-s * sigma)) / s ^ 3 +
    -(py_a3 eta / sigma ^ 3) *
      (24 - (24 + 24 * s * sigma + 12 * s ^ 2 * sigma ^ 2 +
        4 * s ^ 3 * sigma ^ 3 + s ^ 4 * sigma ^ 4) *
        Real.exp (-s * sigma)) / s ^ 5 := by
  unfold C_HS_laplace
  -- Expand: r·(-alpha0 - alpha1(r/sigma) - alpha3(r/sigma)^3)·exp =
  --   (-alpha0)·(r·exp) + (-alpha1/sigma)·(r^2·exp) + (-alpha3/sigma^3)·(r^4·exp)
  have hexpand : ∀ r : ℝ,
      r * (-(py_a0 eta + py_a1 eta * (r / sigma) + py_a3 eta * (r / sigma) ^ 3)) *
      Real.exp (-s * r) =
...
The two integrands agree on [0,sigma): for r < sigma,
...
theorem C_HS_laplace_eq_cHS {eta sigma s : ℝ} (_hsigma : 0 < sigma) :
  ...
  -- Bad set ⊆ {sigma} (null), since for x ∈ uIoc 0 sigma,
  -- x ≠ sigma forces x < sigma and c_HS = poly
