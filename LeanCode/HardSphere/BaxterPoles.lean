...
/-!
# Task BAXTER.11 — pole existence for `1 - ρ·Qhat_complex`

Reduces the zero set of `1 - ρ·Qhat_complex eta sigma rho` (`k ≠ 0`) to the zero set of a clean
...
`Qhat_complex_formula` (`BaxterZeros.lean`, Task `BAXTER.9`) gives `Qhat_complex(k) = A(k) +
...
used implicitly in `BaxterZeros.lean`) collapses `c³·(1-ρQ̂(k))` to `Npoly(k) - Dpoly(k)e^{-ikσ}`
...
`BAXTER.10`, `proof_notes_baxter.md`) is **in progress** — see that file for status.
-/
...
/-- **Cubic** numerator polynomial: `Npoly(k) = ik³ - ρP0k² + iρP1k + 2ρP2`. -/
noncomputable def Npoly (eta sigma rho : ℝ) (k : ℂ) : ℂ :=
  Complex.I * k ^ 3 - (rho : ℂ) * (baxterP0 eta sigma rho : ℂ) * k ^ 2 +
    Complex.I * (rho : ℂ) * (baxterP1 eta sigma rho : ℂ) * k +
    2 * (rho : ℂ) * (baxterP2 eta sigma rho : ℂ)

/-- **Linear** denominator polynomial: `Dpoly(k) = iρ(P1+2P2σ)k + 2ρP2` (its own `c²`-order term
...
/-- **The key algebraic reduction.** `(-ik)³·(1-ρQ̂(k)) = G_baxter(k)` for `k ≠ 0` — clears
...
theorem baxter_cube_mul_F_eq_G (eta sigma rho : ℝ) (hsigma : 0 < sigma) {k : ℂ} (hk : k ≠ 0) :
    (-Complex.I * k) ^ 3 * (1 - (rho : ℂ) * Qhat_complex eta sigma rho k) =
  ...
  rw [hstep1]
  have expand1 : c ^ 3 * (1 - (rho : ℂ) * (Aq + Bq * Complex.exp (-Complex.I * k * sigma))) =
      (c ^ 3 - rho * (c ^ 3 * Aq)) - rho * (c ^ 3 * Bq) * Complex.exp (-Complex.I * k * sigma) := by
    ring
  rw [expand1, hstep2, hstep3, hc3, hc2]
  unfold G_baxter Npoly Dpoly
  rw [← hP0eq, ← hP1eq, ← hP2eq]
  linear_combination (rho : ℂ) * k ^ 2 * Complex.exp (-Complex.I * k * sigma) * hP0P1P2

/-- **`F ↔ G` equivalence.** For `k ≠ 0`, `1-ρQ̂(k)=0 ↔ G_baxter(k)=0`. -/
theorem Qhat_pole_iff_G_baxter_zero (eta sigma rho : ℝ) (hsigma : 0 < sigma) {k : ℂ} (hk : k ≠ 0) :
    1 - (rho : ℂ) * Qhat_complex eta sigma rho k = 0 ↔ G_baxter eta sigma rho k = 0 := by
...
/-! ### Conjugation symmetry (Task `BAXTER.12`, Phase B.2)
...
`BAXTER.10`'s numerical feasibility check found a *log-fixed-point* contraction map with a
...
`BAXTER.11` for the full derivation and numerics. This section formalizes the map and the
...
hypotheses (`hMapsTo`, `hLip`) are exactly `BAXTER.10`'s numerically-validated but not-yet-Lean-
formalized bounds — see `proof_notes_baxter.md` Task `BAXTER.11` for the precise scope. -/
...
/-! ### Magnitude bounds for `Npoly`, `Dpoly` (Task `BAXTER.11`, Phase A.1)
...
the single estimate the rest of `BAXTER.11` builds on. -/
...
theorem Dpoly_eq_affine (eta sigma rho : ℝ) (k : ℂ) :
    Dpoly eta sigma rho k =
      Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ) * k +
        (rho ^ 2 * q_doubleprime_py eta : ℝ) := by
  unfold Dpoly
...
theorem baxterMu_pos {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1) (hsigma : 0 < sigma)
    (hrho : rho ≠ 0) : 0 < rho ^ 2 * q_prime_py eta sigma :=
  mul_pos (by positivity) (q_prime_py_pos heta0 heta1 hsigma)

/-- `ν := ρ²·q_doubleprime_py(η) > 0` for `η∈(0,1), ρ≠0`. -/
theorem baxterNu_pos {eta rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1) (hrho : rho ≠ 0) :
    0 < rho ^ 2 * q_doubleprime_py eta :=
  mul_pos (by positivity) (q_doubleprime_py_pos heta0 heta1)
...
theorem Dpoly_lower_bound {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) (k : ℂ) :
    rho ^ 2 * q_prime_py eta sigma * ‖k‖ - rho ^ 2 * q_doubleprime_py eta ≤
...
/-- **`Dpoly` upper bound**: `‖Dpoly(k)‖ ≤ μ‖k‖ + ν` (Task `BAXTER.14`). -/
theorem Dpoly_upper_bound {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) (k : ℂ) :
    ‖Dpoly eta sigma rho k‖ ≤
      rho ^ 2 * q_prime_py eta sigma * ‖k‖ + rho ^ 2 * q_doubleprime_py eta := by
  rw [Dpoly_eq_affine]
...
theorem Npoly_lower_bound (eta sigma rho : ℝ) (k : ℂ) (hk1 : 1 ≤ ‖k‖)
    (hkbig : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖) :
    ‖k‖ ^ 3 / 2 ≤ ‖Npoly eta sigma rho k‖ := by
  unfold Npoly
  have := norm_cubic_lower_bound (rho * baxterP0 eta sigma rho) (rho * baxterP1 eta sigma rho)
    (2 * rho * baxterP2 eta sigma rho) k hk1 hkbig
...
theorem Npoly_upper_bound (eta sigma rho : ℝ) (k : ℂ) (hk1 : 1 ≤ ‖k‖) :
    ‖Npoly eta sigma rho k‖ ≤
      (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) * ‖k‖ ^ 3 := by
  unfold Npoly
  have := norm_cubic_upper_bound (rho * baxterP0 eta sigma rho) (rho * baxterP1 eta sigma rho)
    (2 * rho * baxterP2 eta sigma rho) k hk1
  convert this using 2
  push_cast
  ring
...
theorem Npoly_hasDerivAt (eta sigma rho : ℝ) (k0 : ℂ) :
    HasDerivAt (Npoly eta sigma rho)
      (3 * Complex.I * k0 ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k0 +
        Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) k0 := by
  have heq : Npoly eta sigma rho =
      fun k : ℂ => Complex.I * k ^ 3 - (rho * baxterP0 eta sigma rho : ℝ) * k ^ 2 +
        Complex.I * (rho * baxterP1 eta sigma rho : ℝ) * k +
        (2 * rho * baxterP2 eta sigma rho : ℝ) := by
    funext k; unfold Npoly; push_cast; ring
  rw [heq]
  exact hasDerivAt_cubic (rho * baxterP0 eta sigma rho) (rho * baxterP1 eta sigma rho)
    (2 * rho * baxterP2 eta sigma rho) k0

/-- **`Npoly'` upper bound.** -/
theorem Npoly_deriv_bound (eta sigma rho : ℝ) (k : ℂ) (hk1 : 1 ≤ ‖k‖) :
    ‖3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
        Complex.I * (rho * baxterP1 eta sigma rho : ℝ)‖ ≤
      (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) * ‖k‖ ^ 2 :=
  norm_cubic_deriv_bound (rho * baxterP0 eta sigma rho) (rho * baxterP1 eta sigma rho) k hk1
...
theorem Dpoly_hasDerivAt (eta sigma rho : ℝ) (k0 : ℂ) :
    HasDerivAt (Dpoly eta sigma rho)
      (Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ)) k0 := by
  have heq : Dpoly eta sigma rho =
      fun k : ℂ => Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ) * k +
        (rho ^ 2 * q_doubleprime_py eta : ℝ) := by
    funext k; exact Dpoly_eq_affine eta sigma rho k
  rw [heq]
  have hid : HasDerivAt (fun k : ℂ => k) (1 : ℂ) k0 := hasDerivAt_id k0
  have hlin : HasDerivAt (fun k : ℂ => Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ) * k)
      (Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ) * 1) k0 := hid.const_mul _
  have hconst : HasDerivAt (fun _ : ℂ => ((rho ^ 2 * q_doubleprime_py eta : ℝ) : ℂ)) 0 k0 :=
    hasDerivAt_const k0 _
  have hcombine := hlin.add hconst
  refine hcombine.congr_deriv ?_
  ring

/-- **`G_baxter`'s derivative**, named for reuse (`BAXTER.12`'s residue formula). -/
noncomputable def G_baxter_deriv (eta sigma rho : ℝ) (k : ℂ) : ℂ :=
  (3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
      Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) -
    ((Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ)) * Complex.exp (-Complex.I * k * sigma) +
...
/-- **`R'/R` bound** (`R := Npoly/Dpoly`): the single estimate the rest of `BAXTER.11`'s
...
theorem R_deriv_ratio_bound {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) {k : ℂ} (hk1 : 1 ≤ ‖k‖)
    (hkN : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖)
    (hkD : 2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ ‖k‖) :
    ‖(3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
          Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) / Npoly eta sigma rho k -
        (Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ)) / Dpoly eta sigma rho k‖ ≤
      (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) / ‖k‖ := by
  ...
  have hXnn : 0 ≤ (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) *
      ‖k‖ ^ 2 := by positivity
  ...
  have heq : (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) *
        ‖k‖ ^ 2 / (‖k‖ ^ 3 / 2) + mu / (mu * ‖k‖ / 2) =
      (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) / ‖k‖ := by
...
/-! ### Branch-safety: `Re(Npoly(k)·conj(Dpoly(k))) > 0` (Task `BAXTER.11`, Phase A.2)
...
theorem Npoly_mul_conj_Dpoly_re_pos {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) {k : ℂ} (hk1 : 1 ≤ ‖k‖)
    (hD : ((rho ^ 2 * q_doubleprime_py eta) +
        (rho ^ 2 * q_prime_py eta sigma) * (|rho * baxterP0 eta sigma rho| +
          |rho * baxterP1 eta sigma rho| + |2 * rho * baxterP2 eta sigma rho|) +
        (rho ^ 2 * q_doubleprime_py eta) * (|rho * baxterP0 eta sigma rho| +
          |rho * baxterP1 eta sigma rho| + |2 * rho * baxterP2 eta sigma rho|)) * ‖k‖ <
      (rho ^ 2 * q_prime_py eta sigma) * (k ^ 2).re) :
    0 < (Npoly eta sigma rho k * (starRingEnd ℂ) (Dpoly eta sigma rho k)).re := by
  have heqN : Npoly eta sigma rho k =
      Complex.I * k ^ 3 - (rho * baxterP0 eta sigma rho : ℝ) * k ^ 2 +
        Complex.I * (rho * baxterP1 eta sigma rho : ℝ) * k +
        (2 * rho * baxterP2 eta sigma rho : ℝ) := by
    unfold Npoly; push_cast; ring
  have heqD : Dpoly eta sigma rho k =
      Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ) * k +
        (rho ^ 2 * q_doubleprime_py eta : ℝ) := Dpoly_eq_affine eta sigma rho k
  rw [heqN, heqD]
  exact norm_cubic_mul_conj_affine_re_pos (rho * baxterP0 eta sigma rho)
    (rho * baxterP1 eta sigma rho) (2 * rho * baxterP2 eta sigma rho)
    (rho ^ 2 * q_prime_py eta sigma) (rho ^ 2 * q_doubleprime_py eta) k hk1
...
/-! ### Differentiability and derivative bound for `baxterPhi` (Task `BAXTER.11`, Phase A.3) -/
...
theorem baxterPhi_hasDerivAt (eta sigma rho : ℝ) (n : ℕ) {k0 : ℂ}
    ...
    HasDerivAt (baxterPhi eta sigma rho n)
      ((Complex.I / (sigma : ℂ)) *
        ((3 * Complex.I * k0 ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k0 +
            Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) / Npoly eta sigma rho k0 -
          (Complex.I * (rho ^ 2 * q_prime_py eta sigma : ℝ)) / Dpoly eta sigma rho k0)) k0 := by
  unfold baxterPhi
  have hN := Npoly_hasDerivAt eta sigma rho k0
  have hD := Dpoly_hasDerivAt eta sigma rho k0
...
theorem baxterPhi_deriv_bound {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) (n : ℕ) {k : ℂ}
    ...
    ‖deriv (baxterPhi eta sigma rho n) k‖ ≤
      (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) /
        (sigma * ‖k‖) := by
  ...
  calc 1 / sigma *
    ...
    _ = (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) /
          (sigma * ‖k‖) := by ring
...
convex, so no sampling is needed, unlike `BAXTER.10`'s numerical check). -/
theorem baxterPhi_lipschitzOnWith {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) (n : ℕ) (k1 : ℂ) (r : ℝ) (M : ℝ) (hM1 : 1 ≤ M)
    (hMball : ∀ k ∈ Metric.closedBall k1 r, M ≤ ‖k‖)
    (hkN : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ M)
    (hkD : 2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ M)
    (hDball : ∀ k ∈ Metric.closedBall k1 r,
      ((rho ^ 2 * q_doubleprime_py eta) +
          (rho ^ 2 * q_prime_py eta sigma) * (|rho * baxterP0 eta sigma rho| +
            |rho * baxterP1 eta sigma rho| + |2 * rho * baxterP2 eta sigma rho|) +
          (rho ^ 2 * q_doubleprime_py eta) * (|rho * baxterP0 eta sigma rho| +
            |rho * baxterP1 eta sigma rho| + |2 * rho * baxterP2 eta sigma rho|)) * ‖k‖ <
        (rho ^ 2 * q_prime_py eta sigma) * (k ^ 2).re) (C : NNReal)
    (hC : (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) /
        (sigma * M) ≤ C) :
  ...
  have hballfacts : ∀ k ∈ Metric.closedBall k1 r,
    ...
    have hkN' : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖ := le_trans hkN (hMball k hk)
    have hkD' : 2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ ‖k‖ :=
    ...
    have hDne : Dpoly eta sigma rho k ≠ 0 := by
      ...
      have hkD2 : 2 * (rho ^ 2 * q_doubleprime_py eta) ≤
          (rho ^ 2 * q_prime_py eta sigma) * ‖k‖ := by
  ...
  have hkN' : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
      |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖ := le_trans hkN hMk
  have hkD' : 2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ ‖k‖ :=
    le_trans hkD hMk
  have hbound := baxterPhi_deriv_bound heta0 heta1 hsigma hrho n hNne hDne hslit hk1' hkN' hkD'
  have hCbound : (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) /
      (sigma * ‖k‖) ≤ (8 + 4 * |rho * baxterP0 eta sigma rho| +
        2 * |rho * baxterP1 eta sigma rho|) / (sigma * M) := by
...
/-! ### Self-map (`MapsTo`) lemma (Task `BAXTER.11`, Phase A.4) -/
...
/-! ### Assembly (Task `BAXTER.11`, Phase A.5)

`baxter_G_zero_exists_for_n` gets a genuine zero of `G_baxter` in `disk(k1,r)` for a single `n`,
**conditional on a "good guess" hypothesis** (`hstep`) — the one piece of `BAXTER.11` not closed
...
partially-open problems (`g0_HS_contact_value_of_oz_h_regularity`, `BAXTER.8`): a genuine,
`sorry`-free conditional theorem, with the remaining gap isolated to one explicit, numerically-
validated (`BAXTER.10`) hypothesis. -/

/-- **`BAXTER.11`, single-`n` existence**: combines `baxterPhi_lipschitzOnWith`,
...
theorem baxter_G_zero_exists_for_n (eta sigma rho : ℝ) (heta0 : 0 < eta) (heta1 : eta < 1)
  ...
  have hDne : Dpoly eta sigma rho k ≠ 0 := by
    ...
    have hmupos := baxterMu_pos heta0 heta1 hsigma hrho
    have hnupos := baxterNu_pos heta0 heta1 hrho
    have hkD2 : 2 * (rho ^ 2 * q_doubleprime_py eta) ≤ (rho ^ 2 * q_prime_py eta sigma) * ‖k‖ := by
      rw [div_le_iff₀ hmupos] at hkD'
      linarith [hkD']
    nlinarith [hDbound, hkD2]
  exact baxterPhi_fixedPt_implies_zero eta sigma rho hsigma n hNne hDne hfp

/-- **`BAXTER.11`, standalone pole family**: extracts the witness-choice function that was
previously buried inside `G_baxter_zeros_infinite`'s proof (via an internal `choose`) as a
reusable named object — `BAXTER.12`'s `h_explicit` (`BaxterResidue.lean`) is stated generically
...
theorem G_baxter_pole_family_exists {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) {r M : ℝ} (hr : 0 < r) (hrspace : r < Real.pi / sigma)
    (hM1 : 1 ≤ M) (N : ℕ) (k1 : ℕ → ℂ)
...
/-- **`BAXTER.11`, infinitude of `G_baxter`'s zero set** — thin corollary of
...
/-- **`BAXTER.11`, final statement**: `1-ρQ̂(k)` has infinitely many zeros — transfers
`G_baxter_zeros_infinite` through `Qhat_pole_iff_G_baxter_zero` (valid for `k≠0`; removing the
single point `k=0` from an infinite set leaves it infinite). This is `BAXTER.11`'s originally-
...
theorem Qhat_complex_zeros_infinite {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    ...
    (hC : (8 + 4 * |rho * baxterP0 eta sigma rho| + 2 * |rho * baxterP1 eta sigma rho|) /
        (sigma * M) ≤ K)
    (hstep : ∀ n, N ≤ n → dist (k1 n) (baxterPhi eta sigma rho n (k1 n)) ≤ r * (1 - K)) :
    {k : ℂ | 1 - (rho : ℂ) * Qhat_complex eta sigma rho k = 0}.Infinite := by
  ...
  have hsub : {k : ℂ | G_baxter eta sigma rho k = 0} \ {0} ⊆
      {k : ℂ | 1 - (rho : ℂ) * Qhat_complex eta sigma rho k = 0} := by
    intro k hk
    obtain ⟨hzero, hne⟩ := hk
...
/-! ### Non-degeneracy at a zero (Task `BAXTER.12`, discharging `Hhat_residue_at_pole`'s
hypotheses)
...
for the second, the mild sign fact `0 ≤ k.im`, matching the upper-half-plane poles `BAXTER.11`
constructs). -/

/-- **`G_baxter_deriv` magnitude at a zero of `G_baxter`** (Task `BAXTER.14`): if
...
theorem G_baxter_deriv_lower_bound_of_zero {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) {k : ℂ} (hzero : G_baxter eta sigma rho k = 0)
    (hk1 : 1 ≤ ‖k‖)
    (hkN : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖)
    (hkD : 2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ ‖k‖)
    (hkS : 4 / sigma ≤ ‖k‖)
    (hkT : 8 * (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) / sigma
        ≤ ‖k‖) :
  ...
  have hsigmaNorm : ‖Npoly eta sigma rho k * (Complex.I * sigma)‖ =
    ...
    rw [norm_mul, norm_mul, Complex.norm_I, Complex.norm_real, Real.norm_eq_abs,
      abs_of_pos hsigma]
    ring
  have hcorrbound : ‖(3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
      Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) -
      Complex.I * (mu : ℝ) * Complex.exp (-Complex.I * k * sigma)‖ ≤
      (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) * ‖k‖ ^ 2 +
        2 * ‖Npoly eta sigma rho k‖ / ‖k‖ := by
    calc ‖(3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
        Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) -
        Complex.I * (mu : ℝ) * Complex.exp (-Complex.I * k * sigma)‖
        ≤ ‖(3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
            Complex.I * (rho * baxterP1 eta sigma rho : ℝ))‖ +
          ‖Complex.I * (mu : ℝ) * Complex.exp (-Complex.I * k * sigma)‖ := norm_sub_le _ _
      _ ≤ (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) * ‖k‖ ^ 2 +
  ...
  have hlow : sigma * ‖Npoly eta sigma rho k‖ -
    ...
    rw [hkey]
    set A : ℂ := (3 * Complex.I * k ^ 2 - 2 * (rho * baxterP0 eta sigma rho : ℝ) * k +
        Complex.I * (rho * baxterP1 eta sigma rho : ℝ)) -
  ...
  have hthresh : (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) *
      ‖k‖ ^ 2 ≤ sigma * ‖k‖ ^ 3 / 8 := by
...
/-- **`G_baxter(-k)` magnitude** (Task `BAXTER.14`): if `Im(k)≥0` and `‖k‖≥1` with `‖k‖` past
...
theorem G_baxter_neg_lower_bound {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) {k : ℂ} (hkim : 0 ≤ k.im) (hk1 : 1 ≤ ‖k‖)
    (hkN : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖) :
    ‖k‖ ^ 3 / 2 - (rho ^ 2 * q_prime_py eta sigma * ‖k‖ + rho ^ 2 * q_doubleprime_py eta) ≤
      ‖G_baxter eta sigma rho (-k)‖ := by
  set mu : ℝ := rho ^ 2 * q_prime_py eta sigma with hmudef
  set nu : ℝ := rho ^ 2 * q_doubleprime_py eta with hnudef
  have hmupos := baxterMu_pos heta0 heta1 hsigma hrho
...
theorem G_baxter_neg_ne_zero_of_large {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    ...
    (hkN : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖)
    (hkU : rho ^ 2 * q_prime_py eta sigma * ‖k‖ + rho ^ 2 * q_doubleprime_py eta < ‖k‖ ^ 3 / 2) :
    G_baxter eta sigma rho (-k) ≠ 0 := by
...
/-! ### `|exp(-ikσ)|` bounds at a zero (Task `BAXTER.14`, A.2)
...
theorem abs_exp_neg_ikn_sigma_lower {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    ...
    (hkN : 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖)
    (hkD : 2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ ‖k‖) :
    ‖k‖ ^ 2 / (4 * (rho ^ 2 * q_prime_py eta sigma)) ≤
      ‖Complex.exp (-Complex.I * k * sigma)‖ := by
  set mu : ℝ := rho ^ 2 * q_prime_py eta sigma with hmudef
  set nu : ℝ := rho ^ 2 * q_doubleprime_py eta with hnudef
  have hmupos := baxterMu_pos heta0 heta1 hsigma hrho
...
theorem abs_exp_neg_ikn_sigma_upper {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    ...
    ‖Complex.exp (-Complex.I * k * sigma)‖ ≤
      2 * (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) * ‖k‖ ^ 2 / (rho ^ 2 * q_prime_py eta sigma) := by
  set mu : ℝ := rho ^ 2 * q_prime_py eta sigma with hmudef
  set nu : ℝ := rho ^ 2 * q_doubleprime_py eta with hnudef
  have hmupos := baxterMu_pos heta0 heta1 hsigma hrho
  have hk0 : 0 < ‖k‖ := lt_of_lt_of_le one_pos hk1
  ...
  have hcoeff_nn : (0:ℝ) ≤
      2 * (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) * ‖k‖ ^ 2 := by positivity
  have hstep1 := mul_le_mul_of_nonneg_left hDlb2 hcoeff_nn
  have hstep2 := mul_le_mul_of_nonneg_right hNub hmupos.le
  have hringeq : 2 * (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
      |2 * rho * baxterP2 eta sigma rho|) * ‖k‖ ^ 2 * (mu * ‖k‖ / 2) =
      (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) * ‖k‖ ^ 3 * mu := by ring
...
/-- **Existential threshold bundle** (Task `BAXTER.14`, A.4): the `hkN`/`hkT` thresholds several
...
theorem exists_hkN_hkT_threshold (eta sigma rho : ℝ) (hsigma : 0 < sigma) :
    ∃ M : ℝ, 0 < M ∧
      (∀ k : ℂ, M ≤ ‖k‖ → 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) ≤ ‖k‖) ∧
      (∀ k : ℂ, M ≤ ‖k‖ →
        8 * (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) / sigma
          ≤ ‖k‖) := by
  set Q1 : ℝ := 2 * (|rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
    |2 * rho * baxterP2 eta sigma rho|) with hQ1
  set Q2 : ℝ := 8 * (3 + 2 * |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho|) /
    sigma with hQ2
...
/-- **Existential `D` bundle** (Task `BAXTER.14`, A.4): `abs_exp_neg_ikn_sigma_upper`'s bound is
...
theorem exists_D_for_exp_neg_bound {eta sigma rho : ℝ} (heta0 : 0 < eta) (heta1 : eta < 1)
    (hsigma : 0 < sigma) (hrho : rho ≠ 0) :
    ∃ D : ℝ, 0 < D ∧ ∀ {k : ℂ}, G_baxter eta sigma rho k = 0 → 1 ≤ ‖k‖ →
      2 * (rho ^ 2 * q_doubleprime_py eta) / (rho ^ 2 * q_prime_py eta sigma) ≤ ‖k‖ →
      ‖Complex.exp (-Complex.I * k * sigma)‖ ≤ D * ‖k‖ ^ 2 := by
  have hmupos := baxterMu_pos heta0 heta1 hsigma hrho
  refine ⟨2 * (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
    |2 * rho * baxterP2 eta sigma rho|) / (rho ^ 2 * q_prime_py eta sigma), by positivity,
    fun {k} hzero hk1 hkD => ?_⟩
  have h := abs_exp_neg_ikn_sigma_upper heta0 heta1 hsigma hrho hzero hk1 hkD
  rw [show 2 * (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
      |2 * rho * baxterP2 eta sigma rho|) / (rho ^ 2 * q_prime_py eta sigma) * ‖k‖ ^ 2 =
      2 * (1 + |rho * baxterP0 eta sigma rho| + |rho * baxterP1 eta sigma rho| +
        |2 * rho * baxterP2 eta sigma rho|) * ‖k‖ ^ 2 / (rho ^ 2 * q_prime_py eta sigma) by ring]
  exact h
