...
| OZ.3 | g₀_HS via OZ Laplace inversion | ◑→**substantially done** (2026-09-02 re-assessment): **(1)** Laplace identity `Ĥ₀=Ĉ_HS·S₀` ✓ axiom-clean (`oz_laplace_identity`/`oz_linearized_identity`); **(2)** contact value `g₀_HS(σ)=(1+η/2)/(1−η)²` ✓ **now a THEOREM** (`g0_HS_contact_value`, footprint std-3 + MA.13 `wiener_causal_resolvent` + MA.15 `radialShell_bounded_injective`; `oz_h_exterior_regularity` retired to a theorem OZFIX.22). ⛔ **(3)** the *full* damped-exponential `g₀_HS(r)` for `r>σ` = `oz_h_eq_spliced_h_explicit`, conditional on `hcollapse` (the ★ core-closure — Fourier route CIRCULAR; genuine obstacle OZFIX.17, L¹-contraction REFUTED for η≳0.13) **and** `hcont_sigma` (OZFIX.7, FALSE-as-stated). Layer (3) is the **OZFIX do-not-attempt boundary**, not an actionable open task | `HardSphere/{PYOZ,JumpAsymptotic}.lean` |
