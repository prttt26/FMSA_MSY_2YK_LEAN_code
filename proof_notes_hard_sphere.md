...
### Task 2.3 *(ex-B.1, moved from Group GAP 2026-07-17 — pure HS Baxter factor)* — Shifted-exponent integral: `∫₀^R r·exp(z(r−R)) dr = (zR−1+exp(−zR))/z²`
...
**Status:** ✓ DONE — `LeanCode/HardSphere/BaxterFactor.lean` (complete):
  - `phi1_shifted_hasDerivAt`: antiderivative `(x/z − 1/z²)·exp(z·(x−R))` via product rule;
...
**`hslope` RESOLVED — the slope has a closed form (2026-07-16).** The earlier note (*"`F'(λ) ≠ 0` is
state-point-dependent, so it stays an explicit numerical hypothesis"*) is **retracted**. Evaluating
`F'` at the cutoff, using only `λ² = (Rᵢ−Rⱼ)²`:

    F'(λ) = 2·(χ₂₂ − χ₁/(4π))          — independent of Rᵢ, Rⱼ, χ₀, χ₂, χ₃

**Mechanism.** Write `F(r) = −P(r)/r − χ₀` (`cHS_core_eq_num`), so `F'(r) = (P(r) − r·P'(r))/r²`.
Group-by-group, at `r = λ`:
- `χ₃` group: `A(λ) − λA'(λ) = (3/2)λ²[−(Rᵢ+Rⱼ)² + 2(Rᵢ²+Rⱼ²) − λ²] = 0` (the bracket is
  `(Rᵢ−Rⱼ)² − λ² = 0`; the `4λ(Rᵢ³+Rⱼ³)` terms cancel first);
- `χ₂` group: `= (Rᵢ+Rⱼ)(λ² − (Rᵢ−Rⱼ)²) = 0`;
- `χ₁` group: `= (1/4)(λ² − (Rᵢ−Rⱼ)²) = 0`;
- `χ₂₂` group: `D(λ) − λD'(λ) = (Rᵢ−Rⱼ)² + λ² = 2λ²`, giving `F'(λ) = 2λ²E/λ² = 2E`, `E := χ₂₂ − χ₁/(4π)`.

So **`hslope ⟺ χ₂₂ ≠ χ₁/(4π)`** — an elementary two-coefficient condition, not a numerical
assumption. This is **sharp**: the kink genuinely *fails* (the FMT DCF is differentiable at `λ`)
exactly when `χ₂₂ = χ₁/(4π)`, so no unconditional kink theorem exists.

*Lean route.* `chiA0/chiA1/chiA2/chiA4` — the `r`-coefficients of `cHS_num` with `E` kept
**abstract** (this is what turns the cancellation into a division-free polynomial identity, so
`linear_combination … * hsq` closes it without `field_simp` fighting the `χ₁/(4π)`);
`cHS_core_eq_laurent` (core `= −(a₀/r + a₁ + a₂r + a₄r³) − χ₀` off `r=0`);
`hasDerivAt_neg_laurent` (that shape's derivative, via `hasDerivAt_inv`/`hasDerivAt_pow`);
`chiA0_eq_of_sq` (**the cancellation**, `a₀ = (2E + a₂ + 3a₄λ²)λ²` mod `λ²−(Rᵢ−Rⱼ)²`);
`chi_slope_value`; then `cHS_core_hasDerivAt_of_sq` → `cHS_core_deriv_at_cutoff`
→ `cHS_core_deriv_at_cutoff_ne_zero_iff` → `cHS_FMT_not_differentiableAt_of_chi`.
The original `cHS_FMT_not_differentiableAt` (taking `hslope`) is kept as the general form.
Verified symbolically (sympy) on both sign branches before formalizing.

**Lean:** `HSMixture/CHSKink.lean` — identifiers task-ID-free (`clampedBelow`, `cHS_core`,
`cHS_FMT`, `cHS_FMT_continuousAt`, `cHS_FMT_not_differentiableAt`, `cHS_core_deriv_at_cutoff`,
`cHS_FMT_not_differentiableAt_of_chi`, …).
**Depends on:** none beyond `get_HS_FMT`'s two-piece definition; only Mathlib.
**Status:** ✓ DONE (2026-07-15; slope closed form 2026-07-16), axiom-clean. Continuity, both
one-sided slopes, **and the slope value** unconditional; the kink now follows from the elementary
`χ₂₂ ≠ χ₁/(4π)`.

**✅ CLOSED 2026-07-17 — the PHYSICAL kink is UNCONDITIONAL (`cHS_FMT_kink_WB`, `CHSKinkWB.lean`, axiom-clean).**
The "sharp ⇒ no unconditional theorem" caveat holds ONLY for abstract χ; for the White-Bear χ's the
hypothesis is now **discharged**. The sharpness above is about `cHS_FMT_not_differentiableAt_of_chi`,
which takes *arbitrary reals* χ. For the actual White-Bear FMT coefficients the combination is
strictly one-signed. Deriving χ₁, χ₂₂ from the FMT Hessian (`get_HS_FMT`: `Φ22 = 6n₂φ`,
`Φ12 = Φ03 = 1/v`, `Φ_v2v2 = −6n₂φ`, etc.) and evaluating `E := χ₂₂ − χ₁/(4π) = ½F'(λ)` gives, after
the vector/scalar pieces combine, the **closed form**

    E  =  n₂·(1/(4v²) − 6π·φ(η))  =  n₂ · g(η) / (12 η² v²),   v := 1−η,
    g(η) := 3η² − 2η − 2(1−η)² ln(1−η)

(`n₂ = Σρ·4πR² > 0`; sympy-derived, and matched to a finite-difference of `get_HS_FMT`'s
overlap branch to 5–6 digits at several states). **`g(η) > 0` for all η ∈ (0,1)** (verified on a
20000-point grid; `g ~ (2/3)η³` as η→0, `g → 1` as η→1), so **`E > 0` for every physical state**
(any diameters, any composition). Hence:
- `χ₂₂ = χ₁/(4π)` occurs **only in the ρ→0 (η=0) ideal-gas limit** — it is the boundary of the
  physical domain, **not an interior coincidence and not an interior special boundary**. (The
  handful of "near-zero" states in the numerical probe were all η≈0.02, i.e. approaching that
  limit, consistent with the `(2/3)η³` vanishing.)
- Therefore the FMT `λ_ij` kink is in fact **unconditional on `0 < η < 1`** (unlike pairs). The
  `χ₂₂ ≠ χ₁/(4π)` hypothesis of `cHS_FMT_not_differentiableAt_of_chi` **can be retired** by wiring
  the concrete WB-χ definitions into Lean, establishing the closed form above, and proving the
  one-variable positivity `g(η) > 0` (elementary: power series `−ln(1−η) = Σ_{k≥1} η^k/k` gives
  `g = 2(1−η)²·Σ_{k≥1} η^k/k − 2η + 3η²` = a manifestly-positive series for η∈(0,1); or via
  `Real.add_one_le_exp`). The one prerequisite the codebase still lacks is the concrete χ₁/χ₂₂ as
  Lean functions of `(η, R_i, R_j, composition)` — `CHSKink.lean` currently carries them abstract.
  (Numerical + symbolic scoping: scratch, `ozfix_chi_probe`.)

**Lean closure (`CHSKinkWB.lean`, 2026-07-17, axiom-clean).** Rather than extract χ₁ and χ₂₂
individually (the polynomial matching leaves χ₂₂ determined only *through* the combination
`E := χ₂₂ − χ₁/(4π)` — `cHS_num` depends on `χ₁` and `E` alone), the closure works directly with `E`:
- `g_wb`, `g_wb_pos` — `g(η)=3η²−2η−2(1−η)²log(1−η) > 0` on `(0,1)`, by nested monotonicity
  (`hasDerivAt_p_wb`: `p′=−log(1−η)`, `p_wb_pos`; `hasDerivAt_g_wb`: `g′=4·p`, so `g′>0`), via
  `strictMonoOn_of_deriv_pos` twice from `g(0)=p(0)=0`. (The tempting `g ≥ η⁴` bound needs a quadratic
  log inequality not in this Mathlib; the monotonicity route uses only `Real.log_neg`.)
- `phiWB`, `E_WB := n₂(1/(4v²) − 6π·φ)`, `E_WB_eq : E_WB = n₂·g(η)/(12η²v²)` (`field_simp`+`ring`,
  log atomic), `E_WB_pos` (from `g_wb_pos`, `n₂>0`).
- **`cHS_FMT_kink_WB`** — instantiates `cHS_FMT_not_differentiableAt_of_chi` at `χ₂₂ := E_WB + χ₁/(4π)`
  (so `χ₂₂−χ₁/(4π)=E_WB>0` discharges the hypothesis) for **any** `χ₀,χ₁,χ₂,χ₃` (the kink is
  independent of them — `F′(λ)=2E`). This covers the physical FMT DCF, whose kink coefficient *is*
  `E_WB`. **`cHS_FMT_kink_WB_fmt`** ties `n₂,η` to the actual FMT densities `wbN2`/`wbN3`
  (`WhiteBearFMT.lean`) under the physical hypotheses `0<wbN2`, `0<η<1`. `E_WB` matches the earlier
  finite-difference of `get_HS_FMT` to 5–6 digits (`ozfix_chi_probe`, scratch).

**⚠ Which HS reference? (open, 2026-07-17).** The kink above is stated and proved for the **FMT**
(White-Bear) reference `get_HS_FMT`. A 2026-07-17 scan (`numerical_notes/results/lambda_knot_minimum.md`,
`probe_sigma_ratio.py`) notes the **Baxter/PY** reference (`get_HS_PY`) appears **C¹** at `λ_ij` — a
one-sided-slope jump of only `~10⁻⁴`, versus the FMT jump `+0.107` at `d₂/d₁=3` and `+0.052` at `6`.
If confirmed, the two HS references differ structurally at the same geometric knot, and OZ.18 applies
to the FMT reference only; a PY-reference kink would be a separate, currently-unverified claim.
(OZ.19's *flatness below* `λ_ij` is functional-agnostic and unaffected either way.)

**Confirmed 2026-07-17** (`FMSA_dp_stepwise_breakpoints.md` §5, via **IB.9**): the first-order Yukawa part `c^(1)` is itself **C¹** at `λ_ij`, so the smoothness of `c_total` there is entirely the HS reference's — `c_total(hs_ref='py')` is C¹, `c_total(hs_ref='fmt')` is C⁰. The kink OZ.18 proves is the FMT weight-convolution corner, and applies to the FMT reference.

---

### Task OZ.19 — `c_HS,ij` is exactly constant on `(0, λ_ij)` — interval flatness, functional-agnostic *(strengthens OZ.18)*

*New 2026-07-17, from the numerical session (`numerical_notes/results/lambda_knot_minimum.md`,
`probe_sigma_ratio.py`, `fmsa_double_prop.py`). A **stronger, cleaner** statement than OZ.18's
endpoint kink: flatness on a whole interval, provable from weight-support geometry alone without
touching the FMT free-energy functional Φ.*

**Statement.** For a hard-sphere mixture and an unlike pair `i ≠ j` with `λ_ij = |d_i − d_j|/2`:

    c^HS_ij(r) is EXACTLY constant on (0, λ_ij).

**Why (functional-agnostic).** Below `λ_ij` the smaller sphere sits entirely inside the larger, so
every FMT weight convolution is `r`-independent there:

    ω₂ⁱ⊛ω₂ʲ = 0,   ω₂ⁱ⊛ω₃ʲ = 4πRᵢ² (Rᵢ<Rⱼ),   ω₃ⁱ⊛ω₃ʲ = (4π/3)·min(Rᵢ,Rⱼ)³,   ∇²(ω₃ⁱ⊛ω₃ʲ) = 0.

Since `c_ij(r) = −Σ_{αβ} (∂²Φ/∂n_α∂n_β)·(ω_α^i ⊛ ω_β^j)(r)` and the Φ-Hessian is evaluated at the
(r-independent) bulk density, all the r-dependence lives in the convolutions — every one of them
constant on `(0, λ_ij)`. So the Hessian factors out and **any** FMT functional Φ gives a flat
`c^HS_ij` there. This is why it is cleaner than OZ.18: OZ.18's slope value `2(χ₂₂−χ₁/(4π))` is
White-Bear-specific, whereas flatness holds for any Φ.

**Relation to OZ.18.** Flatness on `(0, λ_ij)` **implies** OZ.18's C⁰ kink at `λ_ij`: the left
slope is `0`, so the kink is genuine whenever the right slope `≠ 0`. OZ.19 is the interval
statement; OZ.18 is the endpoint corollary (with the sharp White-Bear slope `2(χ₂₂−χ₁/(4π))`).

**Numerical status.** Spread on `(0, λ_ij)`: `0.000e+00` (White Bear FMT, exact), `1.8×10⁻¹¹`
(Baxter/PY, roundoff).

**Proof route (Lean).** Weight-support geometry: each `ω_α^i ⊛ ω_β^j` is constant on the
sub-contact interval (ball-overlap volumes take an `r`-independent value once one ball is contained
in the other), then `c` is a fixed linear combination of them. No Φ, no free-energy functional —
mirrors `get_HS_FMT`/`get_HS_PY`. The `clampedBelow` infra from OZ.18 is reusable. **Effort:**
medium (the convolution-constancy lemmas are the work).

**Depends on.** OZ.18 (clamp infra), the FMT weight definitions.

**Status.** ✓ **DONE 2026-07-17, axiom-clean** (`HSMixture/CHSFlatInner.lean`).
- `cHS_fmt_form` — the FMT DCF as the bilinear form `c(r) = −Σ_{αβ} Φ_αβ·w_αβ(r)` (6 scalar Hessian
  entries + a vector piece), `Φ` abstract.
- **`cHS_flat_of_convs_const`** — the functional-agnostic reduction: if every convolution `w_αβ` is
  constant on a set, `c` is constant there, **for any `Φ`**. Genuinely universally quantified over
  `Φ` *and* the convolution functions — deliberately **not** an `∃`/`rfl` shell (cf. the GAP.8/IB.4
  vacuity lesson): the `Φ`'s provably factor out of a sum of constants.
- **`cHS_fmt_inner_const`** — FMT instantiation on `(0, λ_ij)`. The one physical input, that the
  weight convolutions **are** `r`-constant below `λ_ij` (ball-containment geometry — `get_HS_FMT`'s
  `r < λ` branch: `ω₂⊛ω₂=0`, `ω₂⊛ω₃=4πR²`, `ω₃⊛ω₃=(4π/3)min³`, `∇²=0`), is carried as the explicit
  hypotheses `hconv…`, honestly marked: the codebase encodes those ball-overlap values as closed
  forms (as `get_HS_FMT` does) rather than deriving them from measure theory. The *reduction* — the
  removal of the free-energy functional — is what is proved.

**Cleans OZ.18 (the `then` of the user's request).** OZ.18's kink
(`clampedBelow_not_differentiableAt`) took its **left** slope `0` from `clampedBelow_eqOn_Iic`, i.e.
from the *modelling assumption* that the sub-contact branch equals the clamp `F(λ)`. OZ.19 removes
that: the real FMT DCF is genuinely flat below `λ`, so the left slope is a *proved* fact.
`CHSFlatInner.lean` supplies the decoupled kink:
- `hasDerivWithinAt_Iic_zero_of_flat` — flat on `(0,λ)` + left-continuous into `λ` ⇒ within-`Iic`
  derivative `0` at `λ` (`Ioc 0 λ ∈ 𝓝[Iic λ] λ` via `Set.Ioi_inter_Iic`).
- **`not_differentiableAt_of_flat_below_of_right_deriv`** — flat below `λ` + nonzero right
  derivative ⇒ not differentiable at `λ`, for **any** function. Same slope-mismatch core as
  `clampedBelow_not_differentiableAt`, but with the left slope from OZ.19's flatness rather than the
  `clampedBelow` construction. (The complementary *right*-slope hypothesis `χ₂₂≠χ₁/(4π)` is
  dischargeable under `0<η` — separate, see the OZ.18 section above.)

**⚠ Physical corollary — NOT for Lean.** The finding also observes that the unlike-pair DCF's
interior minimum is pinned *near* `λ_ij` (GCMC argmin/λ = 0.968 for pair (1,2) at σ=[1,2], with no
theory input beyond `d_bh`). But its honesty note is explicit: the minimum is **near, not at**
`λ_ij` — `argmin/λ` drifts systematically 0.89 → 1.06 as `d₂/d₁` goes 2 → 8, tracking the ratio of
the Yukawa slope to the HS kink at `λ`. Formalize only the flatness/corner (exact); the minimum's
location is a Yukawa-shifted consequence and must **not** be stated as an equality.

---

### Task OZ.20 — PY-HS structure factor positivity / "no spinodal": `1 − ρĈ(k) > 0 ∀k`

**Statement.** For all physical `η ∈ (0,1)`, `σ>0`, `ρ` (`η=πρσ³/6`) and all real `k`,
`1 − ρ·radial_fourier(c_HS η σ) k > 0` — the PY hard-sphere structure factor `S(k)=1/(1−ρĈ)` is
finite everywhere (no fluid–fluid spinodal). A *fundamental HS* fact about the PY closure, proved
directly from the analytic `c_HS` (Wertheim–Thiele cubic), not the Baxter machinery.

**Identity with existing tasks.** This IS `BAXTER.16` (task a, `proof_notes_baxter.md`), re-homed
here as the fundamental HS statement. It is also the **real-axis** case of `MA.14`/`POLE.11`'s
Hermite–Biehler root location, tied by the axiom-clean equivalence
`qhat_complex_ne_one_iff_no_spinodal` (`HardSphere/BaxterNoSpinodalEquiv.lean`, 2026-07-18):
`Qhat_complex(k) ≠ 1 ⟺ 0 < 1−ρĈ(k)` for real `k≠0`. ⚠ `MA.14` (closed LHP) is *strictly stronger*
(also needs the open-LHP/winding exclusion); OZ.20 gives only the real-axis conjunct.

**Analytic structure (fully worked out 2026-07-18, `spinodal_sos.py`).** With `t = kσ` and the
Baxter factorization `1−ρĈ = (1−A)² + B²` (`baxter_wiener_hopf_factorization`, `A=∫₀^σ q0·cos`,
`B=∫₀^σ q0·sin`), the exact closed form is a **sum of two squares over a positive denominator**:

  `1 − ρĈ(k) = (N₁(t)² + N₂(t)²) / (t⁶ (1−η)⁴)`,   `N₁ = (1−A)·t³(1−η)²`, `N₂ = B·t³(1−η)²`, with
  `N₁ = t³(1−η)² − 6η(η+2)t·cos t − 18η²t + 12η(2η+1)·sin t`,
  `N₂ = 6η[(η−1)t² + (η+2)t·sin t + 2(2η+1)(cos t − 1)]`.

So **`≥ 0` is FREE** (SOS; = the factorization). **Strict `> 0` ⟺ `(N₁(t), N₂(t)) ≠ (0,0)`** — two
explicit transcendentals with no common positive zero.

**Why the analytic form does NOT give an elementary proof (the honest wall).**
- **Near `t=0`:** `N₁ ~ (1+2η)t³`, `N₂ ~ −(3η/2)t⁴` ⇒ `N₁` dominates, `(N₁,N₂)≠(0,0)`. Fine.
- **Large `t`:** DONE — `one_sub_rho_mul_radial_fourier_c_HS_ne_zero` (`RadialFourierCHS.lean`) gives
  `≠0` for `k ≥ 1+2|ρ|·cHS_bound`, and `1−ρĈ→1`.
- **Dilute `η<(3−√7)/2≈0.177`:** DONE — `Re(1−Q̂)=1−A ≥ 1−M > 0` (`BaxterDiluteSymbol.lean`).
- **The obstacle is the structure-factor PEAK** (`t≈6–9`, finite k), and it is **not compactly
  certifiable uniformly in `η`**: `min_t (1−ρĈ) → 0` as `η→1` (0.80/0.49/0.15/0.0098/0.0005/… at
  `η=0.2/0.4/0.6/0.8/0.9`), the approach to close-packing. **There is NO η-uniform positive lower
  bound**, so no fixed SOS / polynomial certificate exists; any certificate must be η-dependent and
  tight as `η→1`. And `t` appears *both* polynomially and inside `sin/cos`, so this is **not** a
  polynomial-Positivstellensatz/SOS problem (the transcendental coupling doesn't close).

**Verdict.** The user's intuition (analytic form ⇒ provable) is physically sound but the *rigorous*
`∀η<1` positivity is the classical PY-HS result with **no elementary/SOS proof** — the closed form
delivers `≥0` for free and reduces `>0` to "two transcendentals share no zero", which has no uniform
gap.

**DISPOSITION (2026-07-18): registered as the PHYSICS axiom `pyhs_no_spinodal`**
(`HardSphere/BaxterNoSpinodalEquiv.lean`): `∀ k≠0, 0 < 1−ρ·radial_fourier(c_HS) k`.  The equivalent
real-axis forms are now THEOREMS on this single axiom — `qhat_complex_ne_one_of_real` (`Q̂ ≠ 1`, via
`qhat_complex_ne_one_iff_no_spinodal`) and `one_sub_rho_radial_fourier_c_HS_pos` (BAXTER.16, all `k`).
The former closed-LHP MA.14 axiom `G_baxter_ne_zero_on_lower_core` is **RETIRED**, split into this
no-spinodal axiom (real axis) + `baxter_no_open_lhp_pole_core` (the strictly-stronger open-LHP /
winding content, `BaxterHermiteBiehler.lean`); MA.14's `G_baxter_ne_zero_of_im_nonpos` /
`qhat_complex_ne_one_of_im_nonpos` are now theorems on the two.  `#print axioms` of BAXTER.16 =
`{standard three, pyhs_no_spinodal}` only.

**Future route (NOT pursued in this task).** A **verified-computation / interval-arithmetic** proof
parametric in `η` looks the most promising of the two (it does not need an η-uniform gap — one
certifies `N₁²+N₂² > 0` on the compact peak region `t ∈ [t₀, t₁]` per η, with the `η→1` tightness
handled by refining the grid), whereas the argument-principle/winding route is circular at the
boundary (needs this very statement). Recorded as a possible future effort; **this task does not
continue it** — `pyhs_no_spinodal` stays an axiom.
