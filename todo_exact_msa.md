...
| MPhase 2 ⚠FINDING | matching ⇒ c-side constraint **(♦)** `Dt_ij − ∑_l ρ_l·e^{z·edgeLo_jl}·Dt_il·Q̂_jl(z) = 2πK_ij/z` — a **recentering `e^{z·edgeLo_jl}`** vs the posited `MixBHRootUneq` (29′)'s bare `Q̂_jl`. Coincides at equal σ (`edgeLo=0`), differs at unequal σ (σ-power masked by σ=1). Posited (29′) likely needs the recentered transform. **AWAIT decision** before formalizing (♦) as `c_side_constraint_of_exterior` / wiring | ⚠ FLAGGED | `YukawaOZMix/MixtureBlumHoyeDerivation` (docstring) |
