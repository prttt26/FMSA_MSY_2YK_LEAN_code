...
| MPhase 6 full-support ⭐⭐ | **g-side closed for `σ_j<σ_i+2σ_l`.** Inner `t`-integral over **all `ℝ`** ⇒ `hQtransform` = *unconditional* full-line transform (`baxterQ_fullline_transform` = MPhase-1 at `s=z`); the RDF **hard core** (`g_il=0` on `[0,edgeHi_il)`) kills the `t<0` partial-moment correction under overlap `−edgeHi_il<edgeLo_lj` (⟺ `σ_j<σ_i+2σ_l`). Stack `rdfLaplaceMoment_shift_hardcore`→`laplace_conv_full(_of_integrable)`→`mat_laplace_conv_full`→`mat_gside_fold_full`→`gside_33_of_hgside_full`→`MixBHRootUneq_of_oz_full` (retires posited root, no hcore axiom). Range **numerically SHARP** (`msaemix_fullsupport_fold_check.py`: match `≥−σg`, correction real below). Supersedes MP5's ordered-pairs `Ioi 0` fold | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 6 residual | extreme size disparity `σ_j ≥ σ_i + 2σ_l` (some triple in `∑_l`): hard-core correction nonzero, g-side first-principles still open — a much smaller corner than ordered-pairs; c-side (29′) fully general throughout | ○ residual (extreme-disparity only) | `YukawaOZMix/…` |
