...
| MPhase 4 detail | the `Ioi 0` transform in the fold = full-support `qhatMixRuneq_lj` only for `edgeLo_lj≥0`; general full-support conv domain `[edgeLo,∞)` carried as the `hQtransform`/`hQhat` hypothesis (as the scalar does) | ○ residual (hypothesis-level, like scalar) | `YukawaOZMix/…` |
