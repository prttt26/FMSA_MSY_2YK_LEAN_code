...
| C-SIDE FIX ⭐ | **`MixBHRootUneq` (29′) corrected** (`MSAMixtureBHRootUneq.lean:98`): c-side uses recentered `e^{z·edgeLo σ j l}·qhatMixRuneq_jl` (rigorous, =(♦)). `mixBHRootUneq_cSide_of_exterior` proves the corrected (29′) from the exterior closure. Transparent def change (nothing destructures/constructs the root), full lib build green | ✓ DONE, axiom-clean | `YukawaOZMix/{MSAMixtureBHRootUneq,MixtureBlumHoyeDerivation}` |
| g-side (33′) fix RETRACTED | A first WH-consistency recentering of (33′) was **reverted** after finding the g-side Eq (34) structure (`proof_notes_leg3_bh_gside_eq9.md`): `∑_l 2π ĝ_il[δ_lj−ρ_l Q̂_lj]`, `Q̂=∫_λ^∞` (full support, bare `qhatMixRuneq`), `ĝ_il=Gt_il e^{−z·edgeHi_il}` on the WHOLE bracket ⇒ NOT a Q̂-recentering. (33′) left BARE; rigorous form needs the repo-consistent mixture g-source (open, Eq (32) not in Lean) | ⚠ (33′) BARE, rigorous derivation open | `YukawaOZMix/MSAMixtureBHRootUneq` |
...
| MPhase 3 rem. | concrete mixture g-side OZ Eq (32) (matrix g-source + conv domain) ⇒ assemble (33′) (analog scalar `h33_of_gside`). ⚠ g-side has OWN σ-edge factors (`Gt=ĝe^{zσ}`; `Ioi 0` transform vs `qhatMixRuneq_lj`) ⇒ recentering EXPECTED on (33′) too (cross-check confirms c-side finding is systematic) | ○ TODO | `YukawaOZMix/…` |
