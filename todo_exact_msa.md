...
| MPhase 4 rem. | pin the g-source normalization: match `source` to `(A_j+z qp_ij+z²Ct_ij/2)/z²·e^{−z·edgeHi_ij}` + identify `ĝ_il=Gt_il e^{−z·edgeHi_il}`, `Q̂_lj=e^{z·edgeLo(l,j)}qhatMixRuneq_lj` (full-support conv domain `[edgeLo,∞)` not `Ioi 0`) ⇒ the (33′) already numerically pinned (`msaemix_root_recentering_check.py`) | ○ TODO | `YukawaOZMix/…` |
