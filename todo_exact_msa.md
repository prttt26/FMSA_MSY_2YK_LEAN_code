...
| RECENTERING FIX ⭐ | **`MixBHRootUneq` corrected** (`MSAMixtureBHRootUneq.lean:98`): both conjuncts use recentered `e^{z·edgeLo_ab}·qhatMixRuneq_ab` — (29′) rigorous (=(♦)), (33′) by WH-consistency. Full lib **build green (8812)**, transparent (nothing destructures/constructs the root). `mixBHRootUneq_cSide_of_exterior` proves the corrected (29′) conjunct from the exterior closure | ✓ DONE, axiom-clean; g-side (33′) factor pending rigorous derivation | `YukawaOZMix/{MSAMixtureBHRootUneq,MixtureBlumHoyeDerivation}` |
