...
| MPhase 2 rem. | match `matMSAtail` ⇒ (★) `2πK_ij e^{zσ_ij}/z = S_ij − ∑_l ρ_l S_il Q̂_jl(z)`, then amplitude algebra (★)⇒(29′) (unfold `Wt`/`Ct`/`γ`; analog of scalar `h29_of_baxter_exterior`, def-heavy) | ○ TODO | `YukawaOZMix/…` |
