...
|------|-------|--------|-----------|
| MSAEXACT.1 | non-compact Baxter factorization `\|1−ρQ̂(ik)\|²=1−ρĉ_MSA` | ✅ **CLOSED via `exactMSA_hcore` axiom 2026-08-21** — `exactMSA_factorization` = `factorization_of_core` (pure algebra) + the sympy-verified axiom; `#print axioms` = std-3 + `exactMSA_hcore`. ⛔ compact `exactMSA_iff_core` route dead | `YukawaOZ/{MSABaxterKSpace,MSAFullFactorization}` |
| MSAEXACT.6 ⭐ | **the gap** — discharge the `hcore` closure-recovery ring | ⊘ **`ring`-INFEASIBLE, measured 2026-08-22** (clean 323-mono deg-22 slice = `ring` 3h12m/23GB; irreducible per-order reassembly ~weeks/~200GB; atom form worse deg≈35; `native_decide` blocked by noncomputable `MvPolynomial`). ✅ **Fourier layer DISCHARGED** (`YukawaOZ/ExactMSACertificate.lean`, out-of-`defaultTargets` lib): `exactMSA_hcore_of_residual` ⇒ exact `exactMSA_hcore` statement from ONE pure-cos/sin/exp axiom `exactMSA_kspace_residual`. **"Upgradeable/30-min" RETRACTED.** Sole gap of .1, MSAFAM.5/.6@N=1; MSAEMIX.1 = matrix analog | `YukawaOZ/{MSAFullFactorization,ExactMSACertificate}` |
...
| MSAEMIX.1 | matrix factorization (matrix form of MSAEXACT.1) | ✅ **DONE both diameters (modulo the accepted `hcore` axiom = MSAEMIX.4)** — `matMSAmixture_equalDiam` / `matMSAmixture_unequalDiam_of_hcore` via `matMixtureFactorization_of_core` (analog of MSAEXACT.6); (★) cancellation + concrete increment done | `YukawaOZMix/{MSAMixtureCancellation,MSAMixtureFactorization,MSAMixtureBHRoot,MSAMixtureBHRootUneq}` |
| MSAEMIX.2 | `N=1` reduces to MSAEXACT.1 | ✅ **DONE 2026-08-25** — both halves (`msaMixture_reduces_to_scalar_at_fin_one`: factorization → scalar exact-MSA modulus + positivity) | `YukawaOZMix/{MSAMixturePositivity,MSAMixtureFinOne}` |
...
| FOEQ.4 | `first_order_solution_determined` — the `γ=1` equation **plus** the `s`-free support conditions determines `C̃₁`, so (D1)'s answer *is* the derivative once FOEQ.3 supplies the equation | ✅ **DONE 2026-08-10** — both halves: algebraic `firstOrder_dcf_of_oz_deriv` (`C̃₁ = Hinv·(H₁(I−C̃₀))`) **and** WH-uniqueness `firstOrder_khat_unique` (k-space) → `firstOrder_dcf_unique_on_core` (real-space on the open core, via PYE.5 injectivity). (D1)'s output IS the OZ derivative `c_ij(r)` | `Closures/FirstOrderEquivalence.lean`, `Closures/DPClosureMap.lean` |
| FOEQ.5 ⭐⭐ | `msa_solution_differentiable_at_zero_coupling` — the physical MSA root is differentiable in `K` at `K=0`, by the implicit function theorem | ✅ **DONE at N=1, γ=1 (2026-08-19)** — Jacobian ✓ + IFT ✓ + C¹ ✓ ⇒ `MSASolutionFamily.msa_amplitude_differentiableAt_yukawa` (differentiability from `ξ∈(0,1)`, `z>0` alone, via MSAFAM.1–2; Jacobian block-triangular, `det J = (2π)ⁿ∏F₀²`, nonsingular ⟺ `F₀≠0` = `Q0_ne_zero_at_yukawa`). **General N via MSAFAM.7 ← MSAEMIX.6 (now closed 2026-09-01)** | `Closures/…`, `MSASolutionFamily` |
...
| MPhase 2 (♦) ⭐ | `c_side_constraint_of_exterior` — from the exterior closure `hbax` derives the c-side constraint **(♦)** `Dt_ij − ∑_l ρ_l·e^{z·edgeLo_jl}·Dt_il·Q̂_jl(z) = 2πK_ij/z` (matrix analog of scalar `h29_of_baxter_exterior`) | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| RECENTERING FIX ⭐⭐ | **`MixBHRootUneq` both conjuncts recentered** (`MSAMixtureBHRootUneq.lean:98`): (29′) `e^{z·edgeLo σ j l}·Q̂_jl` (=(♦), `mixBHRootUneq_cSide_of_exterior`), (33′) `e^{z·edgeLo σ l j}·Q̂_lj` (opposite orientation, transposed index; Baxter Eq 8 vs 32). **NUMERICALLY CONFIRMED** vs validated `msa_exact_mix.py` (`msaemix_root_recentering_check.py`: (29′) 1.7e-16, (33′) 3.6e-15; bare (33′) misses by ≈5). Full lib build green (8812), transparent def change | ✓ DONE, axiom-clean, numerically pinned | `YukawaOZMix/{MSAMixtureBHRootUneq,MixtureBlumHoyeDerivation}` + `msaemix_root_recentering_check.py` |
| MPhase 3 (CRUX) ⭐ | `mat_laplace_conv` — matrix conv thm `∑_l ρ_l ∫₀^∞ e^{−zr}(∫(r−t)g_il(r−t)Q_lj(t)dt) = ∑_l ρ_l·ĝ_il(z)·Q̂_lj(z)`, **reuses scalar `laplace_conv_eq_rdf_mul_qhat_of_integrable` per species** (Fubini already discharged) + `Finset.sum_congr`. ∑_l contraction = pure linearity (roadmap's crux worry resolved) | ◑ conv thm DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 3/4 g-fold ⭐ | `mat_gside_fold` — g-side analog of scalar `hoz_of_gside`: from the g-side OZ Laplace primitive `hgside_mix`, folds the species-summed RDF⋆Baxter conv via `mat_laplace_conv` ⇒ `2π ∑_l ĝ_il(δ_lj − ρ_l Q̂_lj) = source`. **Discharges the folding crux** | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 4 g-source ⭐ | `gside_33_of_fold` (g-source bookkeeping: `×e^{z·edgeHi_ij}`, `ĝ_il=Gt_il e^{−z·edgeHi_il}`, edge identity `edgeHi_ij−edgeHi_il=edgeLo(l,j)`) + `gside_33_of_hgside` (chains `mat_gside_fold`→`gside_33_of_fold`): **the (33′) conjunct DERIVED from the g-side OZ primitive `hgside_mix` + identifications** — full matrix analog of scalar `h33_of_gside` | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 4 detail | the `Ioi 0` transform in the fold = full-support `qhatMixRuneq_lj` only for `edgeLo_lj≥0`; general full-support conv domain `[edgeLo,∞)` carried as the `hQtransform`/`hQhat` hypothesis (as the scalar does) | ✓ RESOLVED by MPhase 6 (full-`ℝ` fold + hard core) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 5 wire-in ⭐⭐ | `MixBHRootUneq_of_oz` — assembles the derived c-side (29′) (`mixBHRootUneq_cSide_of_exterior`) + g-side (33′) (`gside_33_of_hgside`, via bridge `gside_bracket_sum_eq`) into `MSAMixture.MixBHRootUneq`. **Posited root RETIRED** onto OZ/Baxter primitives — mixture analog of scalar `exactMSA_factorization_of_oz` | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 4/5 | assemble (33′) + wire-in `MixBHRootUneq_of_oz` (retire posited root) + hbax/hgside-style equivalences | ✓ DONE (MP5) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 6 full-support ⭐⭐⭐ | **g-side closed for ALL unequal σ.** Inner `t`-integral over **all `ℝ`** ⇒ `hQtransform` = *unconditional* full-line transform (`baxterQ_fullline_transform` = MPhase-1 at `s=z`); the RDF **hard core** (`g_il=0` on `[0,edgeHi_il)`) kills the `t<0` partial-moment correction under overlap `−edgeHi_il<edgeLo_lj`, which is **AUTOMATIC** (`neg_edgeHi_lt_edgeLo`: `σ_l` cancels ⇒ `=(σ_i+σ_j)/2>0`). Stack `rdfLaplaceMoment_shift_hardcore`→`laplace_conv_full(_of_integrable)`→`mat_laplace_conv_full`→`mat_gside_fold_full`→`gside_33_of_hgside_full`→`MixBHRootUneq_of_oz_full` (overlap discharged from `0<σ`, no side condition, no hcore axiom). Supersedes MP5's ordered-pairs `Ioi 0` fold | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 6 — NO residual | the earlier "extreme size disparity `σ_j≥σ_i+2σ_l`" corner was **spurious** (flipped `edgeLo_lj=(σ_l−σ_j)/2`); real `edgeLo σ a b=(σ_b−σ_a)/2` ⇒ overlap always holds. Confirmed 6.5M random triples, 0 violations (`msaemix_extreme_disparity_check.py`). g-side matches c-side: fully general | ✓ RESOLVED (no corner) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 7 — hbax/hgside discharged ⭐⭐ | the OZ primitives consumed by `MixBHRootUneq_of_oz_full` are **equivalent to the root conditions**: `mat_hbax_iff_cside` (`hbax ⟺ (♦)/(29′)`; reverse `mat_hbax_of_cside` = fold by `mat_exterior_baxter_relation_Dt` + `2πr·matMSAtail=2πK e^{z·edgeHi}e^{−zr}` + edge id) and `mat_hgside_iff_gside` (`hgside_mix ⟺ (33′)`; reverse `mat_hgside_of_gside` = fold conv onto transforms, run keysum/hexpand backward). Matrix analogs of scalar `hbax_iff_h29`/`hgside_iff_h33`. Full unconditional discharge = the ring-infeasible `matExactMSAUnequalDiam_hcore` (honest end of the line) | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 8 — factorization-of-oz ⭐⭐ | mixture analog of scalar `exactMSA_factorization_of_oz`: `matMSAmixture_unequalDiam_of_oz` chains `MixBHRootUneq_of_oz_full` into capstone `matMSAmixture_unequalDiam_of_hcore` ⇒ full MSA factorization from OZ primitives. **Footprint std-3 + `matExactMSAUnequalDiam_hcore`** (one physics axiom, = scalar shape). `_physical_of_oz` = radial-Fourier form (+`matHSexactUnequalDiam_kspace`). End-to-end wire-in parity with scalar | ✓ DONE | `YukawaOZMix/MixtureBlumHoyeDerivation` |
| MPhase 9 — equal-diam root ⭐⭐ | **gap closed**: equal-σ `MixBHRoot` was only POSITED (never derived), unlike `MixBHRootUneq`. Retired via `MixBHRoot_of_oz` = `MixBHRootUneq_of_oz_full` + bridge `MixBHRoot_of_MixBHRootUneq` (equal-σ: `edgeLo=0` ⇒ recentering=1, `qhatMixRuneq_eq_qhatMixR_of_equal`). std-3, no physics axiom. Equal-diam factorization capstone still needs hHS/WH (equal-σ HS not free ftilde algebra) | ✓ DONE, axiom-clean (std-3) | `YukawaOZMix/MixtureBlumHoyeDerivation` |
